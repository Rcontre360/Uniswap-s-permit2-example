...
contract SignatureTransfer is ISignatureTransfer, EIP712 {
    ...
    function getHash(PermitTransferFrom memory permit) external view returns (bytes32) {
        return _hashTypedData(permit.hash());
    }

    function equal(PermitTransferFrom memory permit, bytes32 hash) public view returns (bool) {
        return hash == _hashTypedData(permit.hash());
    }
